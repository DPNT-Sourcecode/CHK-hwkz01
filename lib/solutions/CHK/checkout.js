...
module.exports = function (skus) {
    ...
    for(let i=0; i<skus.length; i++){
        ...
        if(groupMembers.includes(skus[i])){
            groupPrices.push(pricing.get(skus[i])[0].promotionPrice);
        }
    }

    groupPrices.sort((a,b)=>{return b-a})
    ...
    totalCart+=Math.floor(groupPrices.length/3)*45;
    for(let i= Math.floor(groupPrices.length/3)*3 ; i<groupPrices.length; i++){
        totalCart+=groupPrices[i];
    }
